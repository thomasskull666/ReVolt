...
# New sysfs keyboard patch updated by thomasskull666 for ReVolt Kernel
...
if [ ! -f "/data/local/timer_delay" ]; then
  echo 3 > /data/local/timer_delay
fi
cat /data/local/timer_delay > /sys/devices/platform/s3c-keypad/timer_delay

# New keyboard patch for sysfs courtesy of mkasick.  Adjusts the "column-switch delay" allowing for decreased cpu usage and faster response (range: 1-300µs default: 300)
